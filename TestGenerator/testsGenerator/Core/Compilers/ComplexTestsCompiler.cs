...
internal abstract class ComplexTestsCompiler : TestsCompiler
{
    ...
    public override CompilationUnitSyntax GenerateCodeByItem(GenerateItem item)
    {
        if (item.ConstructorParameters == null)
            return base.GenerateCodeByItem(item);

        return SyntaxFactory.CompilationUnit().WithUsings(GenerateUsings(item)).WithMembers(
                                                SyntaxFactory.SingletonList<MemberDeclarationSyntax>(GenerateNamespace(item).WithMembers(
                                                SyntaxFactory.SingletonList<MemberDeclarationSyntax>(GenerateClass(item))))).NormalizeWhitespace();
    }
    ...
    protected override ClassDeclarationSyntax GenerateClass(GenerateItem item)
    {
        ...
        {
            GenerateField(item.ClassName, item.ClassName)
        };
        foreach (ParameterSyntax parameter in item.ConstructorParameters)
        {
            ...
            bool isDependency = parameter.Type is SimpleNameSyntax || parameter.Type is QualifiedNameSyntax;
        ...
        return SyntaxFactory.ClassDeclaration($"{item.ClassName}_Tests").WithModifiers(GenerateModifier(SyntaxKind.PublicKeyword)).WithMembers(new SyntaxList<MemberDeclarationSyntax>(list));
    }

    protected MethodDeclarationSyntax GenerateComplexTestMethod(string name,MethodDeclarationSyntax method)
    {
        return SyntaxFactory.MethodDeclaration(SyntaxFactory.PredefinedType(SyntaxFactory.Token(SyntaxKind.VoidKeyword)),GenerateIdentifier(name))
    ...
    protected BlockSyntax GenerateComplexBlock(MethodDeclarationSyntax method)
    {
        ...
        list.Add(GenerateLocalDeclaration("actual",returnType.ToString(),GenerateInvocation(className, methodName, names.ToArray())));
