...
internal abstract class TestsCompiler
{
    ...
    public virtual CompilationUnitSyntax GenerateCodeByItem(Core.Models.GenerateItem item) =>
        SyntaxFactory.CompilationUnit().WithMembers(SyntaxFactory.SingletonList<MemberDeclarationSyntax>(
                                                                item.NamespaceItem != null? GenerateNamespace(item)
                                                                                    .WithMembers(SyntaxFactory.SingletonList<MemberDeclarationSyntax>(GenerateClass(item)))
                                                                : GenerateClass(item))).NormalizeWhitespace();

    
    protected FileScopedNamespaceDeclarationSyntax GenerateNamespace(Models.GenerateItem item)
    {
        return SyntaxFactory.FileScopedNamespaceDeclaration(SyntaxFactory.IdentifierName($"{item.NamespaceName}.Tests"));
    }
